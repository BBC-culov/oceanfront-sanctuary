...
serve(async (req) => {
  ...
  try {
    ...
    if (type === "booking_confirmation") {
      ...
      // Send to admin
      const adminHtml = adminNotificationHtml({ ...data, guestEmail: data.guestEmail, type: "initial" });
      results.push(
      ...
      // Send to admin
      const adminHtml = adminNotificationHtml({ ...data, guestEmail: data.guestEmail, type: "balance" });
      results.push(
